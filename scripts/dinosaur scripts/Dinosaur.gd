...
class Dinosaur:
	...
	func get_available_actions_by_index():
		return action_list.get_available_actions_by_index(stamina.current_stamina)
	
	
	func match_action(action, dino:Dinosaur, is_sending_action:bool):
		match action.type:
			ActionTypes.ATTACK:
