...
func _ready():
	...
	action_list.actions.append(dinosaur_script.action_script.StaminaBoost.new("0", 0, 100))
	action_list.actions.append(dinosaur_script.action_script.Attack.new("1", 6, 15.0, DamageTypes.SLASHING))
	action_list.actions.append(dinosaur_script.action_script.Attack.new("2", 10, 85.0, DamageTypes.NECROTIC))
	action_list.actions.append(dinosaur_script.action_script.Heal.new("3", 2, 65.0, false))
	'''
	action_list.actions.append(dinosaur_script.action_script.Attack.new("Weak Attack", 1, 15.0, DamageTypes.SLASHING))
	action_list.actions.append(dinosaur_script.action_script.Attack.new("Regular Attack", 3, 35.0, DamageTypes.BLUDGEONING))
	action_list.actions.append(dinosaur_script.action_script.Attack.new("big fuck off attack", 5, 85.0, DamageTypes.NECROTIC))
	...
	action_list.actions.append(dinosaur_script.action_script.StaminaBoost.new("Stim Pack", 3, 10))
	action_list.actions.append(dinosaur_script.action_script.StaminaBoost.new("Stim Pack", 0, 100))
	'''
...
func act_on_action(action, is_source:bool):
	dinosaur.match_action(action, is_source)
	act_on_stat()
...
func act_on_stat():
	...
	if past_stamina.out_of_stamina != stamina.out_of_stamina:
		exhausted_dino() if stamina.out_of_stamina else un_exhausted_dino()
		act_on_stat()
